...
function updated(config) {
  ...
  return function(files, metalsmith, done) {
    ...
    async.forEachOf(updatedFiles, function (file, filename, finished) {
      ...
      var toHash = config.hashMetadata
        ? _.omit(file, ['mode', 'stats'])
        : file.contents;
      try {
        var hash = objectHash(toHash);
      } catch (err) {
        if (config.verbose) {
    ...
    function () {
      jsonfile.writeFileSync(path.join(metalsmith.source(), config.updatedFile), updates, { spaces: 2 });
      removeFiles(files, config);
      done();
    });
  }
